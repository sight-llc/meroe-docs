...
// Strip MDX/markdown/HTML from a search result snippet so the dropdown
// never shows raw syntax like <Note>, **bold**, `code`, or anchor tags.
// Order matters — JSX/HTML tags first, then markdown, then whitespace cleanup.
function stripMarkdown(raw: string): string {
  return raw
    // Remove JSX/HTML tags: <Component prop="x">, </Component>, <br />, etc.
    .replace(/<[^>]+>/g, ' ')
    // Remove markdown links but keep the label: [label](url) → label
    .replace(/\[([^\]]+)\]\([^)]+\)/g, '$1')
    // Remove images: ![alt](url)
    .replace(/!\[[^\]]*\]\([^)]+\)/g, '')
    // Remove bold/italic: ***x***, **x**, *x*, __x__, _x_
                ...
                {results.map((result, index) => {
                  ...
                  return (
                    <li key={result.id}>
                      ...
                      >
                        {/* Title — for page/heading results, content IS the title.
                            For text results, breadcrumbs give context and content is the snippet. */}
                        <div className="font-semibold text-foreground">
                          <HighlightedText
                            text={stripMarkdown(result.type === 'text'
                              ? (result.breadcrumbs?.join(' › ') ?? breadcrumbPath)
                              : result.content
                            )}
                            query={search}
                          />
                        </div>
                        {/* Snippet — only for text results, stripped and truncated */}
                        {result.type === 'text' && result.content && (
                          <div className="text-sm text-muted-foreground mt-0.5 line-clamp-2">
                            <HighlightedText
                              text={stripMarkdown(result.content).slice(0, 160)}
                              query={search}
                            />
                          </div>
                        )}
